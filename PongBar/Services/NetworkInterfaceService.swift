...
enum NetworkInterfaceService {
    ...
    static func isVPNActive() -> Bool {
        guard let store = SCDynamicStoreCreate(nil, "PongBar" as CFString, nil, nil) else { return false }
    ...
    static func getActiveInterface(from snapshot: InterfaceSnapshot? = nil) -> NetworkInterfaceInfo? {
        ...
        if let snapshot, let phys = snapshot.firstPhysicalInterface() {
            ...
            return NetworkInterfaceInfo(
                interfaceName: phys.name, ipAddress: phys.ip,
    ...
    private static func getWiFiInfo() -> NetworkInterfaceInfo? {
